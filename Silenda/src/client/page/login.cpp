...
namespace Silenda
{
	...
	render::MeshFrame LoginPage::OnRender() // executed on the renderer call of page object draw
	{
		...
		m_Mesh->DrawLine({ m_FormPos.x + 1, m_FormPos.y + 2, 0 }, { (m_FormPos.x + 1) + (m_FormLength - 2), m_FormPos.y + 2, 0 }, render::COLOR_DARK_GREEN, render::COLOR_BLACK);
		...
		m_Mesh->DrawUText(L"Display Name:", render::COLOR_DARK_GREEN, render::COLOR_BLACK, { m_FormPos.x + 3, m_FormPos.y + 3, 0 });

		m_Mesh->DrawUText(m_InputText, render::COLOR_DARK_GREEN, render::COLOR_BLACK, { m_FormPos.x + 17, m_FormPos.y + 3, 0 });

		if (m_Timer.elapsed_now<int, std::ratio<1, 1>>() % 2 != 0)
			m_Mesh->DrawUText(L"|", render::COLOR_DARK_GREEN, render::COLOR_BLACK, { (m_FormPos.x + 17) + (short)m_InputText.size(), m_FormPos.y + 3, 0 });
